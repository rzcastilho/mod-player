...
| **D11 — Manual Scenario Sign-Off (tasks.md T058, 2026-09-23): M1–M10 all recorded not executed, host-blocked before any window forms** | Two launches (a plain background job and one via `launchctl asuser 501`, to rule out a foreign security-session artifact of the launching shell), both with a fresh `MODPLAYER_CONFIG_DIR`, left the app's main thread parked 60+ s of active (not idle) CPU inside `modplayer_account::service::AccountService::launch_resolve_session` → `modplayer_secure_store::keyring_store::KeyringSecureStore::get` → `SecKeychainFindGenericPassword` → `mach_msg_trap` (confirmed with `sample <pid> 1`), before `eframe::run_native` is ever reached — `Quartz.CGWindowListCopyWindowInfo(kCGWindowListOptionAll, …)` found no window for the process's PID at any point in either run. The equivalent Keychain lookup run directly (`security find-generic-password -s ModPlayer -a session-credential -w`) returns in under a second with "item not found" (exit 44), so the in-process call is wedging on something the bare CLI path is not — most plausibly an unattended keychain-access-consent prompt from the ad-hoc-signed dev binary with no interactive session to show or dismiss it in. This is the same class of host risk 014 already carried forward as "the sign-in gate" (quickstart.md § 4), just tripped one step earlier — before the Welcome screen (M1) can render, not just before Library/Settings (M2–M10). The block is entirely inside `modplayer_account`/`modplayer_secure_store`, neither of which this feature's source-code tasks (T001–T057) touch. No keychain state was modified and no signed-in state was fabricated to route around it (Governance). | Fabricating a signed-in state or adding a demo mode to reach the window — explicitly forbidden by Governance and out of this feature's scope (FR-017). Declaring the stories verified on automated-test evidence alone — the automated suites (T004–T055) already cover every derivable *value*; they cannot stand in for the *pixel* judgement Governance requires, so the honest record is "not executed," not "substituted." Modifying or clearing the host's real Keychain state to try to unblock the call — an irreversible, consent-requiring action against the operator's actual macOS login keychain, outside this feature's scope and outside what an implementing agent should do unattended. |
