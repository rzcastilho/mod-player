...
- [X] T056 [P] `RUSTUP_TOOLCHAIN=1.95.0 cargo fmt --all --check && cargo clippy --workspace --all-targets --all-features -- -D warnings && cargo test --workspace && cargo deny check`
...
- [X] T058 Execute quickstart.md's manual scenarios M1–M10 (Governance › Manual Scenario Sign-Off): locate the window, drive with `CGEventPost`, capture with `screencapture`, sample with `target/manual-walk/pixel.py`; record pass/deviation with evidence on this task; verify `LaunchStep::Main` is reached before M2–M6/M8–M10, and if the sign-in gate blocks any, record it **not executed** with the reason — never fabricate a signed-in state

  **Result (2026-09-23, host `castilho` macOS 12.7.6 21H1320, worktree `015-control-variants`, commit `050eb0b8`): M1–M10 all recorded NOT EXECUTED. Deeper block than 014's — the window itself never forms, so even M1 (which precedes the sign-in gate) is unreachable.**

  Evidence gathered:
  - `RUSTUP_TOOLCHAIN=1.95.0 cargo build -p modplayer` succeeds (see T056).
  - Launched twice with `MODPLAYER_CONFIG_DIR=$(mktemp -d)` (fresh-first-launch, needed for M1): once as a plain background job, once via `launchctl asuser 501 env … modplayer` to rule out a foreign security-session cause. Both times the process stays alive (`ps` shows `STAT SN`, `%CPU` 8–13 % and climbing `TIME`, i.e. actively spinning, not asleep/blocked-forever-idle) for 60+ s with **zero** windows ever appearing for its PID — checked via `Quartz.CGWindowListCopyWindowInfo` with `kCGWindowListOptionAll` (not just on-screen), across both launches.
  - `sample <pid> 1` during the hang shows the **main thread** parked in `modplayer::main` → `AccountService::launch` → `AccountService::launch_resolve_session` → `KeyringSecureStore::get` → `SecKeychainFindGenericPassword` → … → `mach_msg_trap`, i.e. blocked synchronously on a Keychain/securityd round-trip *before* `eframe::run_native` is ever reached. Window creation is downstream of this call, so it cannot happen while it is stuck.
  - Ruled out "the item is just missing and `security` would say so fast": `security find-generic-password -s ModPlayer -a session-credential -w` (the same lookup, invoked directly) returns in <1 s with `SecKeychainSearchCopyNext: The specified item could not be found` (exit 44) — the CLI path does not hang, so the app's in-process call is wedging on something the CLI doesn't hit (most likely an unattended keychain-access-consent prompt from the ad-hoc-signed dev binary, which has nowhere to render/be dismissed in this non-interactive launch, and there is no host owner note recorded from before that would let a reader confirm otherwise).
  - Full stdout/stderr of both launches are `target/manual-walk/app.log` and `target/manual-walk/app2.log` (gitignored, in-worktree) — no window/launch-step log lines appear in either, only pre-existing plugin `register_*`/`host_busy` timeouts that are themselves a symptom of the main thread never reaching its event loop.
  - Per Governance, no keychain state was modified and no signed-in state was fabricated to work around this.

  This is a **new, more severe instance** of the class of host risk 014 and this feature's own quickstart.md § 4 already carried forward (a session/keychain precondition outside this feature's control gates the manual walk) — it is not caused by anything T001–T057 changed (the blocked call is in `modplayer_account`/`modplayer_secure_store`, untouched by this feature; plan.md § Project Structure confirms this feature touches only `modplayer-ui`).
- [X] T059 If any manual scenario was recorded not-executed, write the deviation back into `quickstart.md` and `research.md`, and note it in `plan.md` § Complexity Tracking, per Governance

  Done: `quickstart.md` § 4 gets a 2026-09-23 update paragraph superseding "M1 is reachable regardless"; `research.md` § R16 gets an addendum with the `sample`/`Quartz`/`security` evidence; `plan.md` § Complexity Tracking gets **D11** with the same evidence and the rejected alternatives (fabricating a signed-in state, declaring the stories verified on automated evidence alone, modifying the host's real Keychain).
